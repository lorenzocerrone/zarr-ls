...
use walkers::common::{SelectionType, ZarrIdentifier};
    ...
    loop {
        ...
        walker = match current_selection {
            ...
            SelectionType::Zarr(node) => {
                match walker {
                    Walker::Dir(_) => Walker::Zarr(ZarrWalker::new(ZarrIdentifier::ZarrNode(node))),
            ...
            SelectionType::Back => {
                match walker {
                    Walker::Dir(file_walker) => {
                        let parent = file_walker.parent();
                        match parent {
                            SelectionType::Dir(path) => Walker::Dir(FileWalker::new(&path)),
                            },
                            _ => panic!("This should not happen"),
                        }
                    }
                    Walker::Zarr(ref zarr_walker) => {
                        let parent = zarr_walker.parent();
                        match parent {
                            SelectionType::Dir(path) => Walker::Dir(FileWalker::new(&path)),
                            SelectionType::Zarr(node) => Walker::Zarr(zarr_walker.clone().next(node)),
                            _ => panic!("This should not happen"),
                        }
                    },
                }
